...
export function Header({ data, currentPath }: { data: WebsiteData; currentPath?: string }) {
  ...
  const navLinks = [
    ...
    { label: 'Rooms', href: `/${slug}/rooms` },
    ...(config.enabledSections.gallery ? [{ label: 'Gallery', href: `/${slug}/gallery` }] : []),
    ...(config.enabledSections.reviews ? [{ label: 'Reviews', href: `/${slug}/reviews` }] : []),
    ...(config.enabledSections.intro ? [{ label: 'About', href: `/${slug}/about` }] : []),
