...
function WebsiteContent() {
  ...
  const [activeTab, setActiveTab] = React.useState<'preview' | 'brand' | 'sections' | 'gallery' | 'reviews' | 'domain' | 'seo' | 'connect'>(
    initialTab || (searchParams.get('mode') === 'connect' ? 'connect' : 'preview')
  );
  ...
  const handleToggleReviewStatus = async (reviewId: string, currentStatus: string) => {
    ...
    const newStatus = isHiding ? 'hidden_for_policy' : 'published';
    let hiddenReason = '';

    if (isHiding) {
      const reasonInput = prompt(
        'Please enter a moderation reason for hiding this guest review (e.g. "Profanity / abusive content", "Spam / advertisement", "Defamatory / unverified third-party dispute"):'
      );
      if (!reasonInput || !reasonInput.trim()) {
        alert('A moderation reason is strictly required by Sena Policy before hiding any guest review.');
        return;
      }
      hiddenReason = reasonInput.trim();
    }
    ...
    try {
      const res = await fetch('/api/reviews', {
        ...
        body: JSON.stringify({
          reviewId,
          status: newStatus,
          hiddenReason: hiddenReason || undefined,
        }),
      });
  ...
  return (
    <div className="flex-1 flex flex-col h-screen overflow-hidden bg-white">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 bg-white">
          ...
          <div className="flex items-center gap-1.5 bg-white border border-[#E8E2DA] p-1 rounded-lg">
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-medium transition ${
                activeTab !== 'connect'
                  ? 'bg-[#191816] text-white shadow-xs'
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#E8E2DA] gap-2">
          <div className="flex items-center gap-4 sm:gap-6 overflow-x-auto whitespace-nowrap">
            {[
              ...
              { id: 'connect', label: 'Sena Connect (API & Embeds)', icon: Code2 },
            ].map((tab) => {
              const Icon = tab.icon;
              return (
                <button
        ...
        {/* TAB 8: SENA CONNECT (DEVELOPER HUB & EMBEDS) */}
        {activeTab === 'connect' && (
          <SenaConnectHub
