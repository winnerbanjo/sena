...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!user?.isActive) return NextResponse.json({ error: 'Please sign in to continue.' }, { status: 401 });
    ...
    return NextResponse.json({ error: 'We could not load your account. Please try again.' }, { status: 500 });
  }
}
